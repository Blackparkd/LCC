		...
		{play, User, Level, {Socket,From}} -> 
			...
			case MatchMaking of
				null ->  % se recebe null, cria nova sala de Warmup com aquele User e põe a sua média com o nivel do User
					Warmup = spawn(fun() -> warmup(Level,[{User, Level, Socket}]) end),
					waitingRoom(Users, [Warmups|Warmup]);
				Warmup -> % se recebe uma sala, manda mensagem à sala para inserir lá dentro ( se tiver espaço)
					Has_Space = hasSpace(Warmup),
					case Has_Space of
						true ->
							Warmup ! {new_player, {User, Level}},
							waitingRoom(Users, Warmups);
						false ->
							Warmup ! {start},
							waitingRoom(Users, Warmups -- [Warmup])
...
matchmaking([],_) -> null;
matchmaking([{Media, Lista}| _], {_,Level}) when ((Level == Media) or (Level == Media + 1) or (Level == Media - 1)) -> 
	{Media,Lista}; % {Media, Lista} -> Warmup
matchmaking([_ |Warmups], User) -> matchmaking(Warmups, User).
...
warmup(Media, Players) ->
	...
	receive
		...
		{new_player, {User,Level}}->
			Avg = newAvg(Media, length(Players), Level),
			warmup(Avg,[{User,Level} | Players])
	end.


hasSpace({Media, Players}) ->
	comp = length(Players),
	case comp of
		4 -> false;
		_ -> true
